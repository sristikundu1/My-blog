      ...
      <div className="w-9/12 mx-auto ">
        <div className="grid grid-cols-4 gap-4 ">
          ...
          <div className="col-span-1 py-6">
             <h1 className='border border-[#6047EC] rounded-lg bg-[#6047EC]/10 text-[#6047EC] text-lg mb-3 font-bold py-5 px-6'>Spent time on read : <span>{totalReadingTime}</span> min</h1>

             <div className="rounded-lg bg-[#111111]/5 p-5">
              <h1 className='text-[#111111] text-lg font-bold text-left'>Bookmarked Blogs : <span>{readBlog.length}</span></h1>
  {readBlog.map((blog) => <ReadBlogList key={blog.id} blog={blog} ></ReadBlogList>)}
             </div>
